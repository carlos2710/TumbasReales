        ...
        If dtCol.Rows.Count > 0 Then
            For i = 1 To dtCol.Rows.Count
                If (contador = dtCol.Rows(i - 1).Item("id_gco").ToString) Then
                    ...
                    formato += "    <h3 class='panel-title' styel='align:center;height:50px;'>"
                    formato += "        <b style='color:#fff'>" + dtCol.Rows(i - 1).Item("descripcion_gco").ToString + "</b>"
                    formato += "    </h3>"
                    formato += "    </div>"
                    formato += "        <div class='colecciones'> "
                    formato += "<img class='avatar-sipan' src='assets/images/sipan1.jpg' alt='' height='20px' widht='20px'><a href='#' onclick='f_Menu(""informationCon.aspx" + "?PER=" + Session("codigo_per") + "&TI=" + tipo_s + "&GC=" + fn.EncrytedString64(dtCol.Rows(i - 1).Item("id_dgc").ToString) + """)'> " + dtCol.Rows(i - 1).Item("descripcion_dgc").ToString + "</a>"
                    formato += "        </div>"
                Else
                    formato += "<div class='colecciones'>"
                    formato += "<img class='avatar-sipan' src='assets/images/sipan1.jpg' alt='' height='20px' widht='20px'><a href='#' onclick='f_Menu(""informationCon.aspx" + "?PER=" + Session("codigo_per") + "&TI=" + tipo_s + "&GC=" + fn.EncrytedString64(dtCol.Rows(i - 1).Item("id_dgc").ToString) + """)'>  " + dtCol.Rows(i - 1).Item("descripcion_dgc").ToString + "</a>"
            ...
            formato += "</div>"
            formato += "</div>"
            Me.conservaciones.InnerHtml = formato
        End If
